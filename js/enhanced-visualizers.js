...
class UIManager {
    ...
    createToolbar() {
        this.toolbar = Utils.createElement('div', { className: 'visualizer-toolbar' });
        ...
        const leftControls = Utils.createElement('div', { className: 'visualizer-left-controls' });
        
        // Create right controls group
        const rightControls = Utils.createElement('div', { className: 'visualizer-right-controls' });
    ...
    createVisualizerSelector() {
        const visualizerSelector = Utils.createElement('select', { className: 'visualizer-selector' });
        ...
        const visualizerOptions = [
            ...
            { id: 'particles', label: 'Particle Constellation' },
            { id: 'nebula', label: 'Audio Nebula' },
        ...
        visualizerOptions.forEach(option => {
            const optionElement = Utils.createElement('option', { value: option.id });
    ...
    createColorSchemeSelector() {
        const colorSchemeSelector = Utils.createElement('select', { className: 'color-scheme-selector' });
        ...
        Object.keys(this.colorSchemes).forEach(scheme => {
            const optionElement = Utils.createElement('option', { value: scheme });
    ...
    addFullscreenButton() {
        ...
        this.fullscreenButton = Utils.createElement('button', { className: 'visualizer-fullscreen-button' });
...
class SpectrumVisualizer extends VisualizerBase {
    constructor(core) {
        super(core, 'spectrum');
        this.options = { ...Config.visualizers.spectrum };
...
class Waveform3DVisualizer extends VisualizerBase {
    constructor(core) {
        ...
        this.options = { ...Config.visualizers.waveform3d };
...
class VortexVisualizer extends VisualizerBase {
    constructor(core) {
        super(core, 'vortex');
        this.options = { ...Config.visualizers.vortex };
        this.options.time = 0;
        this.options.points = [];
    }
...
class FractalVisualizer extends VisualizerBase {
    constructor(core) {
        super(core, 'fractal');
        this.options = { ...Config.visualizers.fractal };
        this.options.rotation = 0;
        this.options.noiseOffset = 0;
        this.options.fractalOffset = {x: 0, y: 0};
        this.options.julia = {real: -0.8, imag: 0.156};
...
class TerrainVisualizer extends VisualizerBase {
constructor(core) {
super(core, 'terrain');
this.options = { ...Config.visualizers.terrain };
this.options.heightMap = [];
this.options.heightHistory = [];
}

init() {
this.options.heightMap = Array(this.options.gridSize).fill(0).map(() => Array(this.options.gridSize).fill(0));
this.options.heightHistory = [];
...
// Simple 2D top-down approach (for better performance)
...
// Create height map directly from audio data
const heightMap = Array(gridSize).fill(0).map(() => Array(gridSize).fill(0));

// Fill height map with audio data
for (let x = 0; x < gridSize; x++) {
    // Map x to frequency index
    const fftIndex = Math.floor((x / gridSize) * (fftData.length / 4));
    const value = Utils.normalizeFFTValue(fftData[fftIndex]);
    
    // Use the same value for the entire column with some variation
    for (let y = 0; y < gridSize; y++) {
        // Add noise based on position
        const noise = Math.sin(x * 0.2 + y * 0.2) * 0.2;
        
        // Get height value (0-1)
        heightMap[x][y] = value + noise * energy.mid;
    }
}

// Store current height map
this.options.heightMap = heightMap;

// Get water level based on bass
const waterLevel = 0.2 + energy.bass * 0.1;
...
for (let x = 0; x < gridSize; x++) {
    for (let y = 0; y < gridSize; y++) {
        ...
        if (isWater) {
            // Water
            const waterIntensity = 0.5 + (waterLevel - height) * 2;
            color = Utils.hexToRgba(this.core.colorScheme.secondary, 0.6 + energy.bass * 0.3);
        } else {
            // Land - color gradient from low to high
            const normalizedHeight = (height - waterLevel) / (1 - waterLevel);
            color = Utils.lerpColor(this.core.colorScheme.primary, this.core.colorScheme.accent1, normalizedHeight);
        }
...
ctx.strokeStyle = `rgba(255, 255, 255, ${0.1 + energy.bass * 0.1})`;
ctx.lineWidth = 1;
...
for (let i = 0; i < 10; i++) {
    ...
    for (let x = 0; x < canvas.width; x += 10) {
        const waveHeight = Math.sin(x * 0.01 + time + i) * 5 * (1 + energy.bass);
        const y = (waterLevel * canvas.height) + i * 15 + waveHeight;
...
// Add height indicators (contour lines)
for (let h = waterLevel + 0.1; h < 1; h += 0.1) {
    ctx.strokeStyle = `rgba(255, 255, 255, ${0.05 + (h - waterLevel) * 0.1})`;
    ctx.lineWidth = 1;
    
    for (let x = 0; x < gridSize - 1; x++) {
        for (let y = 0; y < gridSize - 1; y++) {
            // Check if this cell crosses the height threshold
            ...
            if ((h1 < h && h2 > h) || (h1 > h && h2 < h) ||
                ...
                // Draw contour line segment
                const cellX = x * cellWidth;
                const cellY = y * cellHeight;
                
                ctx.strokeRect(
                    cellX + cellWidth * 0.25, 
                    cellY + cellHeight * 0.25, 
                    cellWidth * 0.5, 
                    cellHeight * 0.5
                );
            }
...
if (energy.bass > 0.3) {
    ...
    for (let i = 1; i <= 5; i++) {
        const radius = (i / 5) * maxRadius * energy.bass;
        const opacity = 0.3 - (i / 5) * 0.2;
...
this.options = { ...Config.visualizers.holographic };
...
constructor(core) {
super(core, 'caustics');
this.options = { ...Config.visualizers.caustics };
...
this.options = { ...Config.visualizers.particles };
...
constructor(core) {
super(core, 'nebula');
this.options = { ...Config.visualizers.nebula };
...
this.ui = new UIManager(this.container, this.colorSchemes, {
    ...
    onDebugToggle: (isDebug) => { window.DEBUG_VISUALIZERS = isDebug; }
});
...
if (existingContainer) {
    ...
    this.container = Utils.createElement('div', { id: 'enhanced-visualizers-container' });
...
this.visualizers.set('terrain', new TerrainVisualizer(this));
...
this.contexts.set(id, canvas.getContext('2d', { willReadFrequently: true }));
